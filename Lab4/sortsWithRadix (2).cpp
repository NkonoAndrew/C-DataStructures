...
const int MAXSIZE = 50000000;  // you might need to set this to a lower value
...
void sort8(record  nums[], int len);  // heapsort
void sort9(record  nums[], int len);  // radix
...
int main()
{
    ...
    void(*sorts[]) (record[], int) = { sort0, sort1, sort2, sort3, sort4, sort5, sort6, sort7, sort8, sort9 };
    int numSorts = 10, startId, endId, loops;
    char response, ocode;
    bool printIt;
    cout << "Start with which sort ID (0 to " << numSorts - 1 << ") ? ";
    cin >> startId;
    startId = max(0, startId);
    cout << "End with which sort ID (0 to " << numSorts - 1 << ") ? ";
    cin >> endId;
    endId = min(numSorts - 1, endId);
    cout << "Initially ascending, shuffled, descending or random order ('a', 's','d', 'r') ? ";
    cin >> ocode;
    cout << "Print the arrays ('y' or 'n') ? ";
    cin >> response;
    printIt = response == 'y';
    ...
    while (!done) {
        ...
        if (size <= 0) done = true;
            ...
            for (int i = startId; i <= endId; i++) {
                cout << "Starting sort" << i << "...";
                double time = timeIt(*sorts[i], size, loops, printIt, ocode);
                cout << "done. Time" << i << " = " << time;
                if (loops > 1) cout << " = " << time / loops << " per call ";
                cout << endl << endl;
            }
        }
    }
...
// selection sort (this is a minsort, to contrast with the textbook's maxsort)
...
// heapsort
void sort8(record a[], int len) {
    heapSort(a, len);
}

// radix sort
void sort9(record* nums, int len) {
    const int BASE = 10;
...
double timeIt(void(*sort)(record[], int), int size, int loops, bool printIt, int ocode) {
    ...
    return tfine / 1000.0;  // elapsed time in seconds
}
